...
class MyDataset(Dataset):
    def __init__(self, h5py_file, is_train=True):
        ...
        for group_id in file:
            raw_points = np.array(file[str(group_id) + "/lidar_scan"])
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(raw_points)
            pcd = pcd.voxel_down_sample(voxel_size=5.0)
            pcd.estimate_normals()
            scan = np.asarray(pcd.normals, dtype="float32")
            normals = np.asarray(pcd.points, dtype="float32")
            self.scans.append(np.hstack([scan, normals]))
            self.poses.append(np.array(file[str(group_id) + "/global_pose"]).astype("float32"))
    # 获取单条数据
    def __getitem__(self, index):
        ref_pose = self.poses[index]
        src_pose = self.poses[index + 1]
        rel_pose = np.linalg.inv(ref_pose) @ src_pose
        sample = {"points_raw": self.scans[index], "points_ref": self.scans[index], "points_src": self.scans[index+1], "transform_gt": rel_pose[0:3, 0:4]}
        # print(sample["points_ref"].shape, sample["points_src"].shape)
        return sample
    # 数据集长度
    def __len__(self):
...
def main():
    ...
    train_set = MyDataset(
        "/home/li/study/intelligent-vehicles/cooper-AR/large_scale_pointcloud_matching/tools/carla_scan.h5")
    val_set = MyDataset(
        "/home/li/study/intelligent-vehicles/cooper-AR/large_scale_pointcloud_matching/tools/carla_scan.h5")
...
def validate(data_loader, model: torch.nn.Module, summary_writer, step: int):
...
def run(train_set, val_set):
    ...
    for epoch in range(0, _args.epochs):
        ...
        for train_data in train_loader:
            # train_data: label, idx, points_raw, points_src, points_ref, crop_proportion, transform_gt
            print(train_data["transform_gt"])
            global_step += 1
            ...
            if global_step % _args.validate_every == 0:  # Validation loop. Also saves checkpoints
                ...
                val_score = validate(val_loader, model, None, global_step)
