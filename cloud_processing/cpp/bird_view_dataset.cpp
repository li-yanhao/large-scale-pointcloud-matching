...
DEFINE_int32(min_voxel_size, 4, "min_voxel_size");
DEFINE_double(submap_scale, 100, "submap_scale");
DEFINE_double(submap_resolution, 0.1, "submap_resolution");
...
struct Submap {
        ...
        center(calculate_center(*cloud))
    {
        ...
        auto grid = make_grid_with_indices(*cloud, FLAGS_submap_scale, FLAGS_submap_resolution);
        int grid_size = int(FLAGS_submap_scale / FLAGS_submap_resolution);
        int segment_id = 0;
        for (int i = 0; i < grid_size; ++i) {
            for (int j = 0; j < grid_size; ++j) {
                auto& indices = grid.at(i, j);
                if (indices.size() > FLAGS_min_voxel_size) {
                    typename pcl::PointCloud<PointT>::Ptr segment_cloud(new pcl::PointCloud<PointT>());
                    for (auto index : indices) {
                        segment_cloud->push_back(cloud->points[index]);
                    }
                    pcl::PointXYZ segment_center((i + 0.5) * FLAGS_submap_resolution + center.x -
                                                         FLAGS_submap_scale / 2,
                                                 (j + 0.5) * FLAGS_submap_resolution + center.y -
                                                         FLAGS_submap_scale / 2,
                                                 0);

                    Segment segment{segment_id++, segment_cloud, segment_center};
...
Correspondence::Correspondence(const Submap<PointT>& submap_i, const Submap<PointT>& submap_j)
{
    ...
    for (const auto& segment_i : submap_i.segments) {
        for (const auto& segment_j : submap_j.segments) {
            bool is_segments_close = fabs(segment_i.center.x - segment_j.center.x) < FLAGS_submap_resolution &&
                                     fabs(segment_i.center.y - segment_j.center.y) < FLAGS_submap_resolution;
            if (is_segments_close)
