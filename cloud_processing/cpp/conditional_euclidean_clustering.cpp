...
typedef pcl::PointXYZI PointTypeIO;
typedef pcl::PointXYZILPS PointTypeFull;
...
bool customCondition(const PointTypeFull& point_a, const PointTypeFull& point_b, float squared_distance)
{
  // Eigen::Map<const Eigen::Vector3f> point_a_normal = point_a.getNormalVector3fMap (), point_b_normal = point_b.getNormalVector3fMap ();
  // std::cout << "a normal z: " << point_a_normal.z() << std::endl;
  // std::cout << "b normal z: " << point_b_normal.z() << std::endl;
    if ((point_a.planarity > 0.7 && point_b.planarity > 0.7)) {
        return (true);
    }
    return (false);
}
...
int main (int argc, char** argv)
{
    ...
    std::cerr << "Computing descriptors...\n", tt.tic ();
    pcl::copyPointCloud (*cloud_out, *cloud_with_normals);
    // pcl::NormalEstimation<PointTypeIO, PointTypeFull> ne;
    pcl::DescriptorEstimation<PointTypeIO, PointTypeFull> de;
    de.setInputCloud (cloud_out);
    de.setSearchMethod (search_tree);
    de.setRadiusSearch (1);
    de.compute (*cloud_with_normals);
